...
description: 'Guia para implementar animações nativas com a View Transition API do React 19+. REQUER React 19+ ou Next.js App Router (já inclui React canary). Use quando o usuário quiser adicionar transições de página, animar mudanças de rota, criar animações de elemento compartilhado, animar entrada/saída de componentes, reordenação de listas, navegações direcionais (avançar/voltar), ou integrar view transitions em Next.js. Use também quando mencionar: view transitions, startViewTransition, ViewTransition, transition types, ou animações entre estados de UI sem bibliotecas externas.'
license: MIT
metadata:
  author: vercel
  version: "1.0.0"
